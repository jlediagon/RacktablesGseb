...
(
	...
	'9.12.3.1.3.1626'=> array
	(
		'dict_key' => 2427,//Cisco Nexus 9372PX
		'text' => 'RJ45 1000BASE T',
		'processors' => array ('nexus-any-QSFP+','nexus-any-QSFP-split','nexus-any-10000SFP+'),
...
function doSNMPmining ($object_id, $snmpsetup)
{
	...
	$endpoints = findAllEndpoints ($object_id, $objectInfo['name']);//récupère l'adresse IP dans l'onglet IP du Switch 
	showWarning ($endpoints[0]);
	showWarning ($endpoints);

	foreach ($endpoints as $ligne){showError("$ligne");}
	...
	//foreach ($objectInfo as $key => $value){showWarning("{$key}=>{$value}");}
...
function doSwitchSNMPmining ($objectInfo, $device)
{
	...
	foreach ($device as $kay => $ligne){showError("{$kay}=>{$ligne}");}
	//showWarning ($device);
