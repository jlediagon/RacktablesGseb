...
function renderRow ($row_id)
{
	...
	$summary['U total'] = $rowInfo['sum'];	
	$summary['U reserved'] = getUreservedforRow($row_id);				 
	$summary['U occupés'] = getUbusyforRow($rackList, $row_id);
	$summary['U libre'] = ($rowInfo['sum']-getUbusyforRow($rackList, $row_id));
	$summary['Espacement total (fib)'] = getRSUforRow ($rackList,"eTotal");
	$summary['Espacement utilisé (fib)'] = getRSUforRow ($rackList,"eUsed");
	$summary['Espacement libre (fib)'] = getRSUforRow ($rackList,"eFree");
	$summary['% used'] = getProgressBar (getRSUforRow ($rackList,"prcentUsed"));
...
function renderRack ($rack_id, $hl_obj_id = 0)
{
	...
	for ($i = $rackData['height']; $i > 0; $i--)
	{
		...
		for ($locidx = 0; $locidx < 3; $locidx++)
		{
			...
			/////////////////code rajoutée////////////////
			/*echo '<br>';
				print ($rackData[$i][$locidx]['object_id']);	
			echo '</br>';*/
			$statut=reservedColor($rackData[$i][$locidx]['object_id']);
			if ($statut == 6)
			{
				$state ='R';
				echo "<td class='atom state_R";
...
function renderRackInfoPortlet ($rackData)
{
	...
	foreach (getAttrValuesSorted ($rackData['id']) as $record)
		if ($record['id'] != 27 && $record['id'] != 29 && $record['value'] != '')
			$summary['{sticker}' . $record['name']] = formatAttributeValue ($record);
	$valp = 0.42638475839384;
	$summary['% used'] = getProgressBar (getRSUforRack ($rackData));
	$summary['% Prises utilisées'] = getProgressBar (getPDUforRack($rackData));
	...
	if ($rackData['comment'] != '')
	{
		...
		startPortlet ('Etat des PatchPanel');
			testPatchPanel ($rackData);	
		finishPortlet ();
}
