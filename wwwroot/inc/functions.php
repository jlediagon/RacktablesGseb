...
// Same for row.
function getRSUforRow ($rowData,$espac)
{
	if (!count ($rowData))
		return 0;
	$counter = array ('A' => 0, 'U' => 0, 'T' => 0, 'W' => 0, 'F' => 0);
	...
	switch ($espac)
	{
		case "prcentUsed":
			...
			break;
		case "eFree":
			return ($counter['T'] + $counter['W'] + $counter['U'] + $counter['F'])-($counter['T'] + $counter['W'] + $counter['U']);
			break;
		case "eTotal":
			return ($counter['T'] + $counter['W'] + $counter['U'] + $counter['F']);
			break;
		case "eUsed":
			return ($counter['T'] + $counter['W'] + $counter['U']);
			break;	
...
function listPortObj ($obj_id)
{
		...
		/*for ( $i = 0 ; $i <= $nbrport-1 ; $i++)
		{
			echo '<br>';
			echo ' Porta : '.$res[$i]['porta'].'';
			echo ' Portb : '.$res[$i]['portb'].'';
			echo ' id : '.$res[$i]['id'].'';
			echo '</br>';
			if($res[$i]['porta'] == $res[$i]['id'])
			{
				$counter++;
			}
			if($res[$i]['portb'] == $res[$i]['id'])
			{
				$counter++;
			}	
		}*/
